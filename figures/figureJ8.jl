...
function gpPlot(ligandName, cellType, gp, time)
    ...
    ind = 0
    while ind < 2
        valDF = filter(row -> row["Bivalent"] .== ind, filtFrame)

        valDataDF = valDF[!, [:Dose, :Time, :Mean]]
        valDataDF = groupby(valDataDF, [:Time, :Dose])
        ...
        plt.plot!(doseVec, μs[ind, :], c = colors[ind], xscale = :log10, ylims = (0,5), yticks = 0:0.5:5, label = valency, legend = :bottomright, legendfontsize = 5, markersize = 5)

        if length(log10.(valDF[valDF.Bivalent .== ind, :].Mean_mean .+ 1)) > 0
            plt.scatter!(doseVec, log10.(valDF[valDF.Bivalent .== ind, :].Mean_mean .+ 1), c = colors[i], xscale = :log10, label = "")
        end
        ind = ind + 1
    end
    ...
    plt.plot!(doseVec, μs[i, :], c = colors[i], xscale = :log10, ylims = (0,5), yticks = 0:0.5:5, label = valency, legend = :bottomright, legendfontsize = 5, markersize = 5)

    if length(log10.(realDataDF[realDataDF.Bivalent .== ind, :].Mean_mean .+ 1)) > 0
        plt.scatter!(doseVec, log10.(realDataDF[realDataDF.Bivalent .== ind, :].Mean_mean .+ 1), c = colors[i], xscale = :log10, label = "")
    end"""

    """for (i, ITtime) in enumerate(biv)
        xMat = zeros(length(doseVec), length(intrinsLevels) + 2)
        xMat[:, 1] .= log10.(doseVec)
        xMat[:, 2] .= ITtime
        xMat[:, 3:size(xMat, 2)] .= repeat(intrinsLevels, outer = [1, length(doseVec)])'
    ...
    """if compType != "none"
        ...
        μsComp = zeros(length(valency), length(doseVec))
        σ²sComp = similar(μs)

        #pl1 = plt.plot()

        for (i, ITtime) in enumerate(valency)
            xMatComp = zeros(length(doseVec), length(intrinsLevelsComp) + 2)
